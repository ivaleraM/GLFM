...
% modified on June 09, 2010
  ...
  if Dm ~= 0;
    ...
    b_psi = cell(1,length(M));
    loglik_per_n.discrete = zeros(size(data.discrete));
    for d = 1:length(M)
      idx = sum(M(1:d-1))+1:sum(M(1:d));
      psi_d = psi(idx,:);
      smPsi = exp(myLogSoftMax([psi_d; zeros(1,N)]));
      ...
      val_tmp = 0.5*obs_d_1*log(2*pi)*length(idx)...
                + 0.5*logdet(params.invA{d})*obs_d_1...
                + 0.5*sum(y_m.*(params.A{d}*y_m),1) - obs_d_1.*c;
      logMult_n = logMult_n + val_tmp;
      loglik_per_n.discrete(d,:) = val_tmp;
    end
  end
