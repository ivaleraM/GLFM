...
idxs_nans<-which(is.nan(data_prost$X[,2]))
idxs_nonnans<-setdiff(1:(length(data_prost$X[,2])),idxs_nans)
mm <- min(data_prost$X[idxs_nonnans,2])
MM <- max(data_prost$X[idxs_nonnans,2])
h <- hist(data_prost$X[idxs_nonnans,2], breaks=100,prob=TRUE)
...
# Mejorar la grafica de histograma y probability mass function
 ...
 # mm <- params$t_1(mm)
  #MM <- params$t_1(MM)
 h <- hist(data_prost$X[idxs_nonnans,4], breaks=100)
# h$density <- h$counts/sum(h$counts)
...
idxs_nans <- which(is.nan(data_prost$X[,4]))
 ...
 mm <- min(data_prost$X[idxs_nonnans,4])
 MM <- max(data_prost$X[idxs_nonnans,4])
 #h$density <- h$counts/sum(h$counts*(h$mids[2]-h$mids[1]))
 pdf_val<-GLFM_computePDF(data_prost,Zp,output$hidden,output$params,4)
 plotcols<-c('red','blue','green','pink','yellow')
 h <- hist(data_prost$X[idxs_nonnans,4], breaks=100,prob=TRUE)
 lines(pdf_val$xd,pdf_val$pdf[1,], xlab = expression("x"[d]),ylab="",col=plotcols[1],type="l")
 #lines(pdf_val$pdf[1,], xlab = expression("x"[d]),ylab="",col=plotcols[1],type="l")
 par(new=T)
 for(d in 2:5){
   lines(pdf_val$xd,pdf_val$pdf[d,], xlab = expression("x"[d]),ylab="",col=plotcols[d],type="l")
   #lines(pdf_val$pdf[d,], xlab = expression("x"[d]),ylab="",col=plotcols[d],type="l",axes=F)
   par(new=T)
   #print("Press return to continue")
 }
 #par(new=T)
