...
for bb=1:length(BASEDIR_ALL)
clearvars -except BASEDIR_ALL bb FILEN_ALL
...
load([res_path, 'SimCluster' BASEDIR1 '/' (FILEN1) '_missP10_it1.mat']);
[N,D]=size(res.X_true);
Nit=10;
Err=zeros(9,Nit);
Err_D=zeros(D,9,Nit);
...
for Pmissing=[10:10:90]
    ...
    for it=1:Nit
        ...
        load([res_path, 'SimCluster' BASEDIR1 '/' (FILEN1)  '_missP' num2str(Pmissing) '_it' num2str(it) '.mat']);%BMF
